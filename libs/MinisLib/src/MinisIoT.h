...
// ─── Extension callback ────────────────────────────────────────────────────────
/**
 * Called when MyCastle sends an extension request to this device.
 *
 * @param id     Request UUID — pass to extRespond()
 * @param op     Operation name  (e.g. "key_press", "click")
 * @param params Operation parameters as JSON object
 *
 * Always call extRespond(id, ...) after processing.
 */
using MinisExtCallback =
    std::function<void(const char* id, const char* op, JsonObjectConst params)>;
...
public:
    ...
    /**
     * Register an IoT entity.
     *
     * Entities are announced in the hello message so MyCastle renders the
     * appropriate UI controls. Writable entities (MinisSwitch, MinisNumber,
     * MinisButton, MinisSelect) intercept incoming commands whose name matches
     * the entity id and call the entity callback — no manual ackCommand() needed.
     *
     * Read-only entities (MinisSensor, MinisBinarySensor) are only metadata
     * declarations; their values must be reported via sendTelemetry() using
     * the entity id as the metric key.
     *
     * The caller retains ownership — the entity must outlive MinisIoT.
     * Typical usage: declare entities as global variables.
     *
     * @param entity  Pointer to a MinisEntity (or subclass) instance.
     */
    void addEntity(MinisEntity* entity);

    /**
     * Register a device-side extension handler.
     * Call before begin(). The extension is announced in hello and
     * subscribed automatically on every MQTT connect.
     *
     * @param extType  Extension type string, e.g. "vkbd", "vmouse"
     * @param cb       Callback invoked when a request arrives
     */
    void addExtension(const char* extType, MinisExtCallback cb);
    ...
    /**
     * Announce device presence to MyCastle on connect.
     ...
     * MUST be called after every extension callback, even on failure.
     *
     * @param extType   Extension type string (must match addExtension call)
     * @param id        Request UUID (from the callback)
     * @param ok        true → success, false → failure
     * @param dataJson  Optional result JSON string, e.g. "{\"x\":100,\"y\":200}"
     * @param errorCode Error code string (only used when ok == false)
     * @param errorMsg  Error description (only used when ok == false)
     */
    bool extRespond(const char* extType, const char* id, bool ok,
...
private:
    ...
    // entityId → entity pointer (not owned)
    std::map<std::string, MinisEntity*> _entities;

    // extType → callback
    std::map<std::string, MinisExtCallback> _extensions;
    // extType → req topic string  (minis/{user}/{device}/ext/{type}/req)
    std::map<std::string, std::string>      _extReqTopics;
    // extType → res topic string
    std::map<std::string, std::string>      _extResTopics;
