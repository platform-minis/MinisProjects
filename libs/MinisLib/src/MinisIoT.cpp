...
void MinisIoT::addEntity(MinisEntity* entity) {
    if (entity) _entities[entity->id()] = entity;
}

void MinisIoT::addExtension(const char* extType, MinisExtCallback cb) {
    _extensions[extType] = cb;
    char buf[192];
    snprintf(buf, sizeof(buf), "minis/%s/%s/ext/%s/req", _userId, _deviceId, extType);
    _extReqTopics[extType] = buf;
    snprintf(buf, sizeof(buf), "minis/%s/%s/ext/%s/res", _userId, _deviceId, extType);
    _extResTopics[extType] = buf;
}
...
bool MinisIoT::sendHello() {
    ...
    if (!_extensions.empty()) {
        JsonArray exts = doc["extensions"].to<JsonArray>();
        for (const auto& kv : _extensions) {
            JsonObject e = exts.add<JsonObject>();
            e["type"]    = kv.first.c_str();
            e["enabled"] = true;
        }
    }

    if (!_entities.empty()) {
        JsonArray ents = doc["entities"].to<JsonArray>();
        for (const auto& kv : _entities) {
            JsonObject e = ents.add<JsonObject>();
            kv.second->toJsonObject(e);
        }
    }
...
bool MinisIoT::extRespond(const char* extType, const char* id, bool ok,
                           const char* dataJson, const char* errorCode,
                           const char* errorMsg) {
    if (!_connected) return false;
    auto it = _extResTopics.find(extType);
    if (it == _extResTopics.end()) return false;

    JsonDocument doc;
    doc["id"] = id;
    doc["ok"] = ok;
    if (ok && dataJson && dataJson[0] != '\0') {
        JsonDocument data;
        if (!deserializeJson(data, dataJson)) {
            doc["data"] = data.as<JsonObjectConst>();
        }
    }
    if (!ok && (errorCode || errorMsg)) {
        JsonObject err = doc["error"].to<JsonObject>();
        if (errorCode) err["code"]    = errorCode;
        if (errorMsg)  err["message"] = errorMsg;
    }

    char buf[512];
    serializeJson(doc, buf, sizeof(buf));
    return _publish(it->second.c_str(), buf);
}
...
void MinisIoT::_handleMessage(const char* topic, const char* data, int dataLen) {
    ...
    // ── Extension request ─────────────────────────────────────────────────────
    for (const auto& kv : _extReqTopics) {
        if (kv.second == topic) {
            auto cbIt = _extensions.find(kv.first);
            if (cbIt == _extensions.end()) return;

            JsonDocument doc;
            if (deserializeJson(doc, data, dataLen)) return;

            const char* id = doc["id"] | "";
            const char* op = doc["op"] | "";
            JsonObjectConst params = doc.as<JsonObjectConst>();
            _log("EXT [%s] op=%s id=%s", kv.first.c_str(), op, id);
            cbIt->second(id, op, params);
            return;
        }
    }
    ...
    auto entityIt = _entities.find(name);
    if (entityIt != _entities.end()) {
        _log("ENTITY CMD [%s] id=%s", name, id);
        try {
            entityIt->second->handleCommand(pl);
            ackCommand(id, true);
        } catch (...) {
            ackCommand(id, false, "entity handleCommand threw");
        }
        return;
    }
...
void MinisIoT::_mqttEventHandler(void* arg, esp_event_base_t /*base*/,
    ...
    switch (static_cast<esp_mqtt_event_id_t>(eventId)) {
        ...
        case MQTT_EVENT_CONNECTED:
            ...
            for (const auto& kv : self->_extReqTopics) {
