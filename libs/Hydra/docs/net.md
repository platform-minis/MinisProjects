...
> **`UdpClient` nie jest w drzewie.** Wersja, która przyszła z gałęzią, wołała
> typy `Endpoint` i `Datagram` oraz `INetworkInterface::resolve()`, których nie
> ma po żadnej stronie — kod nie kompilował się nawet u siebie. Został wycofany
> przy scalaniu i czeka w historii (`git show <commit>:libs/Hydra/src/net/UdpClient.cpp`).
> Wszystko, co ten dokument o nim mówi, jest opisem zamiaru, nie stanu kodu.
